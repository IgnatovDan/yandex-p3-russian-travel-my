...
test.page`../section-pages/cover.html`(`Cover_hover screenshot test 900`, async t => {
  await t.resizeWindow(900, 100);
  await t.hover('.cover').wait(1000);
  await t.expect(await compareScreenshot(t, 'cover_hover_900.png')).ok();
});

test.page`../section-pages/footer.html`(`Footer screenshot test 900`, async t => {
  await t.resizeWindow(900, 100);
  await t.expect(await compareScreenshot(t, 'footer_900.png')).ok();
});
